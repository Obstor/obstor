...
func TestECPublicKey(t *testing.T) {
	for _, curve := range []elliptic.Curve{elliptic.P224(), elliptic.P256(), elliptic.P384(), elliptic.P521()} {
		t.Run(curve.Params().Name, func(t *testing.T) {
			...
			decoded, err := key.DecodePublicKey()
			if err != nil {
				t.Fatal(err)
			}
			if !privateKey.PublicKey.Equal(decoded) {
				t.Fatal("decoded public key differs from the original")
			}
			...
			for _, test := range []struct {
				...
				{"missing-x", "", key.Y},
				{"missing-y", key.X, ""},
				{"invalid-base64-x", "!", key.Y},
				{"invalid-base64-y", key.X, "!"},
				{"short-x", base64.RawURLEncoding.EncodeToString(point[2 : 1+size]), key.Y},
				{"short-y", key.X, base64.RawURLEncoding.EncodeToString(point[2+size:])},
				{"long-x", base64.RawURLEncoding.EncodeToString(point[:1+size]), key.Y},
				{"long-y", key.X, base64.RawURLEncoding.EncodeToString(point[size:])},
				{"unequal-widths", base64.RawURLEncoding.EncodeToString(point[1:size]), base64.RawURLEncoding.EncodeToString(point[size:])},
				{"off-curve", base64.RawURLEncoding.EncodeToString(make([]byte, size)), base64.RawURLEncoding.EncodeToString(make([]byte, size))},
			} {
				t.Run(test.name, func(t *testing.T) {
					invalid := key
					invalid.X, invalid.Y = test.x, test.y
					if _, err := invalid.DecodePublicKey(); err != errMalformedJWKECKey {
						t.Fatalf("expected malformed EC key error, got %v", err)
					}
				})
			}
		})
...
func TestPublicKey(t *testing.T) {
	...
	if key0, ok := keys[0].(*ecdsa.PublicKey); !ok {
	...
	} else if ecdhKey, ecdhErr := key0.ECDH(); ecdhErr != nil {
		t.Fatalf("Failed to convert key[0] to ECDH: %v", ecdhErr)
	} else if !bytes.Equal(ecdhKey.Bytes(), []byte{0x4, 0x30, 0xa0, 0x42, 0x4c, 0xd2,
		...
		0x8, 0x17, 0x23}) {
		t.Fatalf("Bad key[0] ECDH bytes, got %v", ecdhKey.Bytes())
